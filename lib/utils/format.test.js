...
describe('Format utils', () => {
  ...
  describe('#dateTime', () => {
    describe('#toUnix', () => {
      ...
      it('should respond null with something that cannot be parsed', () => {
    ...
    describe('#toDateString', () => {
      it('should accept unix timestamps in integer format and give back the date string', () => {
        should(format.dateTime.toDateString(1563385338)).be.eql('17/07/2019');
      });
      it('should accept ms timestamp in integer format and give back the date string', () => {
        should(format.dateTime.toDateString(1563385338000)).be.eql('17/07/2019');
      });
      it('should accept unix timestamps in string format and give back the date string', () => {
        should(format.dateTime.toDateString('1563385338')).be.eql('17/07/2019');
      });
      it('should respond null with something that cannot be parsed', () => {
        should(format.dateTime.toDateString('foo')).be.eql(null);
      });
      it('should respond null with something null or undefined', () => {
        should(format.dateTime.toDateString(null)).be.eql(null);
        should(format.dateTime.toDateString(undefined)).be.eql(null);
      });
    });
  });

  describe('#uppercaseFirstLetter', () => {
    it('should uppercase the first letter of each word', () => {
      should(format.uppercaseFirstLetter('padme')).be.eql('Padme');
      should(format.uppercaseFirstLetter('pADME')).be.eql('Padme');
      should(format.uppercaseFirstLetter('qui-gon gin')).be.eql('Qui-Gon Gin');
      should(format.uppercaseFirstLetter('padme amidala')).be.eql('Padme Amidala');
    });
    it('should do nothing for numbers except converting to string', () => {
      should(format.uppercaseFirstLetter(123)).be.eql('123');
      should(format.uppercaseFirstLetter('123')).be.eql('123');
    });
    it('should convert null, undefined and multiple blank chars to an empty string', () => {
      should(format.uppercaseFirstLetter(null)).be.eql('');
      should(format.uppercaseFirstLetter(undefined)).be.eql('');
      should(format.uppercaseFirstLetter('    ')).be.eql('');
      should(format.uppercaseFirstLetter('    ')).be.eql('');
    });
  });
});
