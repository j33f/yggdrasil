...
const
  ...
  {assign, isArray, castArray, fill, zipObject} = require('lodash');
...
class Storage {
      ...
      return this.redis.set(data)
        ...
        .then(resp => {
          ...
          if (this.yggdrasil.socketIoController) {
            if (newObject) {
              this.yggdrasil.socketIoController.broadcast('create/' + data.index + '/' + data.collection, resp);
            } else {
              this.yggdrasil.socketIoController.broadcast('update/' + data.index + '/' + data.collection, resp);
            }
          }
      ...
      .then(oldDocument => {
        ...
        return this.redis.set(setData)
          ...
          .then(resp => {
            ...
            if (this.yggdrasil.socketIoController) {
              this.yggdrasil.socketIoController.broadcast('update/' + data.index + '/' + data.collection + '/' + resp._id, resp);
            }
  ...
  async delete(params) {
    ...
    if (this.yggdrasil.socketIoController) {
      this.yggdrasil.socketIoController.broadcast('delete/' + data.index + '/' + data.collection + '/' + data.id, {deleted: true, status: 'ok'});
    }
