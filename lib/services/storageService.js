...
class Storage {
  constructor(yggdrasil, index, collection) {
    ...
    this._safeId = this.yggdrasil.storage.mongo._safeId;
  }

  /**
   * Format a suitable data object
   * @param params
   * @returns {{index: *, collection: *, _id: *, body: *}}
   * @private
   */
  _formatData(params) {
    params = params || {};
    ...
    finalData._id = params.objectId || params._id;
  ...
  _setObject(data, event = 'create') {
    return this.redis.set(data)
      ...
      .then(resp => {
        ...
        this.yggdrasil.fire(`${event}/${data.index}/${data.collection}`, resp);
   ...
   * @param params if contains property _id, set is in update mode, if no property _id, set is in create mode
  ...
  set(params) {
    ...
    if (!data._id) {
      // no _id given : its a new ohject to store
      newObject = true;
      data._id = this.yggdrasil.uuid();
    }
    ...
    return this.get({
      ...
      .catch(() => {
        // the document do not exists (was submitted with a forced _id)
        return this._setObject(data);
      });
  }
   ...
   * @param {object} params - object that specifies criteria for get. mandatory properties : index (takes collection index if unspecified), _id
   * @returns {object} - object specified in params. Rejects if no index, or no _id specified
   */
  get(params) {
    const data = this._formatData(params);

    const dataValidation = this._checkParameters(data, ['index', 'collection', '_id']);
    if(dataValidation instanceof Error) {
      return Bluebird.reject(dataValidation);
    }
  ...
  delete(params) {
    ...
    return this.redis.delete(data)
      ...
      .then(() => {
        this.yggdrasil.fire('delete/' + data.index + '/' + data.collection + '/' + data._id);
        return ({
          status: 'ok',
          data: data
        });
    ...
    return this.list(params)
      .then(response => {
        ...
        if (response.list.length === 1) {
          response.element = response.list[0];
   ...
   */
  delCache (key) {
    return this.redis.rawDelete(key);
  }
