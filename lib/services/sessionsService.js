...
class SessionsService {
  ...
  isRevoked (req, payload, done) {
    ...
    req.yggdrasil.sessionsService.isRevokedPromise(payload)
      .then(() => done(null, false))
      .catch(() => done(null, true));
  }

  /**
   * Same as isRevoked but uses promises (used by socketioJwt and this.isRevoked)
   * @param payload the given payload to check
   * @returns {Promise|Promise<T | Promise>}
   */
  isRevokedPromise (payload) {
    if (!payload.jti) {
      this.yggdrasil.logger.warn('no JTI in JWT', JSON.stringify(payload));
    ...
    return this.yggdrasil.storageService.getCache(this.sessionId(payload.jti, this.yggdrasil.config.iss))
      ...
      .catch(() => {
        this.yggdrasil.logger.warn('Revoked JWT', JSON.stringify(payload));
        Bluebird.reject(new Error('Revoked JWT'));
      });
  }
