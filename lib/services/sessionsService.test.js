...
describe('Sessions service', () => {
  beforeEach(() => {
    storageServiceStub = {
      setCache: sandbox.stub().resolves(),
      getCache: sandbox.stub().resolves(),
      delCache: sandbox.stub().resolves()
    };
    ...
    JWTLibStub = {
      isMock: true,
      sign: sandbox.stub().returns('SignedJWT'),
      decode: sandbox.stub().returns('DecodedJWT')
    };

    uuidStub = sandbox.stub().returns('ThisIsAnUUID');
  ...
  describe('#isRevoked', () => {
    it('should call the callback with true (revoked) and no error if the payload does not contains a jti property', (done) => {
      testService.isRevoked({yggdrasil}, {}, (err, response) => {
        should(err).be.null();
    ...
    it('should call the callback with true (revoked) and no error if the payload is valid but not found in cache', (done) => {
      yggdrasil.storageService.getCache = sandbox.stub().rejects();
      testService = new SessionsService(yggdrasil, JWTLibStub, uuidStub);

      testService.isRevoked({yggdrasil}, {jti: 'ThisIsAJTI'}, (err, response) => {
        should(err).be.null();
        should(response).be.true();
    ...
    it('should call the callback with false (valid) and no error if the payload is valid, found in cache and not empty', (done) => {
      yggdrasil.storageService.getCache = sandbox.stub().resolves({foo: 'bar'});
      testService = new SessionsService(yggdrasil, JWTLibStub, uuidStub);
  ...
  describe('#isRevokedPromise', () => {
    ...
    it('should call the callback with true (revoked) and no error if the payload is valid, found in cache but empty', () => {
      yggdrasil.storageService.getCache = sandbox.stub().resolves('');
      testService = new SessionsService(yggdrasil, JWTLibStub, uuidStub);
  ...
  describe('#revoke', () => {
    it ('Should call yggdrasil.storageService.delCache', () => {
      ...
      should(storageServiceStub.delCache).have.been.calledWith(['session', yggdrasil.config.iss, payload.jti].join(':'));
    });
  });
});
