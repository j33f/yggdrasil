...
xdescribe('Storage Service', () => {
  ...
  describe('#_setObject', () => {
    beforeEach(() => {
      yggdrasil.fire.resetHistory();
      stubbedRedis.set.resetHistory();
      stubbedMongo.set.resetHistory();
  ...
  describe('#set', () => {
    ...
    it('should call get then _setObject if the _id is present and the object exists, asking for replace => replace object', async () => {
      service._setObject = sinon.stub().resolves();
      service.get = sinon.stub().resolves({document: {toto: 'tutu'}});
