...
async function configureYggdrasilForFunctionalTesting (yggdrasil, _yggdrasil) {
  ...
  Object.keys(yggdrasil.repositories).forEach(key => {
    if (key !== 'plugins' && key !== 'yggdrasil') {
      _yggdrasil.logger.info(`configuring repository "${key}" to have "testing_${_yggdrasil.repositories[key].index}" as index`);
  ...
  Object.keys(yggdrasil.repositories.plugins).forEach(key => {
    Object.keys(yggdrasil.repositories.plugins[key]).forEach(repo => {
      _yggdrasil.logger.info(`configuring repository "${key}" to have "${_yggdrasil.repositories[key].index}" and "${_yggdrasil.repositories[key].collection}" as index/collection`);
