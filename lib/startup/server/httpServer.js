...
async function spawnHttpServer (yggdrasil) {
  yggdrasil.fire('log', 'info', '🚦  Spawning the HTTP server...');
  ...
  if (yggdrasil.server.domain === 'localhost') {
    yggdrasil.fire('log', 'info', '🔓  Serving from localhost: no security');

    http = require('http');
    server = http.createServer(yggdrasil);
  } else {
    yggdrasil.fire('log', 'info', '🔐  Not serving from localhost: https is used');
