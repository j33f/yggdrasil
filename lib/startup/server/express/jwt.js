...
async function JWT (yggdrasil) {
  ...
  yggdrasil.use(async (req, res, next) => {
    if (req.session && req.session.userId) {
      const user = new req.yggdrasil.lib.businessObjects.security.user(yggdrasil, req.session.userId);
      return user.get()
        .then(user => {
        ...
        .catch(e => {
          req.yggdrasil.logger.warn(`JWT: This user looks to not exist: #${req.session.userId}`);
          console.error(e);
