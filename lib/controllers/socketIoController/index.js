...
class SocketIoController {
  ...
  addListeners(socket, listeners) {
    listeners.forEach(listener => {
      socket.on(listener.event, async (data, cb) => {
        return await this._listenerHandler(socket, listener, data, cb);
      });
    });
  }
  ...
  async _listenerHandler(socket, listener, data, cb) {
    ...
    try {
      ...
      await this.yggdrasil.sessionsService.isRevokedPromise(this.yggdrasil, socket.decoded_token);
    } catch (e) {
    ...
    let user = new this.yggdrasil.lib.models.security.user(this.yggdrasil, socket.decoded_token.userId);
    ...
    try {
      ...
      this.yggdrasil.logger.log('socketio', ['SIO', listener.event, ': OK', end[0] + 's ' + Math.round(end[1] / 1000000) + 'ms'].join(' '));
      return true;
    } catch (e) {
  ...
  _accessErrorHandler(e, end, listener, cb) {
    ...
    if (e.message === 'Invalid JWT' || e.message === 'Revoked JWT') {
      this.yggdrasil.logger.log('socketio-crit', ['SIO', listener.event, ': Invalid or Revoked JWT', end[0] + 's ' + Math.round(end[1] / 1000000) + 'ms'].join(' '));
      cb({
        rejected: true,
        reason: 'JWT',
      ...
      this.yggdrasil.logger.log('socketio-crit', ['SIO', listener.event, ': FORBIDDEN', end[0] + 's ' + Math.round(end[1] / 1000000) + 'ms'].join(' '));
      ...
      this.yggdrasil.logger.log('socketio-error', ['SIO', listener.event, ': Execution Error', end[0] + 's ' + Math.round(end[1] / 1000000) + 'ms'].join(' '));
      this.yggdrasil.logger.error(e);
      cb({
        rejected: true,
        reason: 'REQUEST',
  ...
  _callErrorHandler(e, end, listener, cb) {
    ...
    // access log
    this.yggdrasil.logger.log('socketio-error', ['SIO', listener.event, ': ERROR', e.status || 'CALL', e.message, end[0] + 's ' + Math.round(end[1] / 1000000) + 'ms'].join(' '));
    if (!e.status) {
      this.yggdrasil.logger.error(e);
    }
  }
  ...
  broadcast(event, message = null) {
    this.yggdrasil.logger.log('socketio', ['Event broadcasted:', event, 'with message:', JSON.stringify(message)].join(' '));
