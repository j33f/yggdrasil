...
module.exports = [
  ...
  /**
   * Get distinct values for Zones
   */
  {
    event: 'users/getZones',
    cb: (socket, yggdrasil, session, user, data, fn) => {
      return yggdrasil.lib.controllers.users.getDistinct(yggdrasil, session, user, {
        key: 'zones'
      })
        .then(result => {
          fn(result);
  ...
  /**
   * Get distinct values for attachmentZone
   */
  {
    event: 'users/getAttachmentZones',
    cb: (socket, yggdrasil, session, user, data, fn) => {
      return yggdrasil.lib.controllers.users.getDistinct(yggdrasil, session, user, {
        key: 'attachmentZone'
      })
