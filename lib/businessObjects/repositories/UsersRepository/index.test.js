...
describe('Users repository', () => {
  ...
  describe('#get', () => {
    it('should not call the auth repo getForUserId method and add credentials to the user body when noCredentials is true', async () => {
      storageService.get = sandbox.stub().resolves({_id: 'id', body: {_id: 'id', foo: 'bar'}});
      yggdrasil.repositories.auth.getForUserId = sandbox.stub().resolves({foo: 'bar'});
    ...
    it('should resolves with the user if there a re no credentials', async () => {
      storageService.get = sandbox.stub().resolves({_id: 'id', body: {_id: 'id', foo: 'bar'}});
      yggdrasil.repositories.auth.getForUserId = sandbox.stub().rejects();
  ...
  describe('#list', () => {
    it('should inject the credentials into the returned list', async () => {
      storageService.list = sandbox.stub().resolves({list: [{_id: 'id', foo: 'bar'}]});
      yggdrasil.repositories.auth.getForUserId = sandbox.stub().resolves({baz: 'qux'});
