 ...
 */
const canAccessUser = (yggdrasil, user, repoUser) => {
...
const reduceList = (result, yggdrasil, session) => {
  ...
  result.list.forEach(user => {
    if (canAccessUser(yggdrasil, session.user, user)) {
      response.push(user);
    }
  });
  return {
 ...
 */
index.me = async (yggdrasil, session) => {
 ...
 * @return {*}
 */
index.setClientOption = (yggdrasil, session, data) => {
 ...
 * @returns {boolean}
 */
index.canList = async (yggdrasil, session, data) => {
  ...
  let
    canList = (internalPolicies.filter(policy => session.user.data.policies.includes(policy)).length > 0 || externalPolicies.filter(policy => session.user.data.policies.includes(policy)).length > 0), // if the user have none of those policies, he cannot list other users
    authorizedCombination = (directionPolicies.filter(policy => session.user.data.policies.includes(policy)).length > 0), // tells if current user policy matches policy from query: true when connected user can list users requested policy
    matches;
  ...
  if (canList && !authorizedCombination && data.query.policies) {
    ...
    if (data.query.policies.length > 0) {
      matches = combinationMatcher(session.user.data.policies, data.query.policies);
 ...
 * @param query{object}
 */
index.pickOne = async (yggdrasil, session, query) => {
  let users = await index.list(yggdrasil, session, {
    query: query
  });
 ...
 * @returns {*}
 */
index.list = (yggdrasil, session, data) => {
...
index.get = async (yggdrasil, session, data) => {
  const userId = data.objectId || data.userId || data;
  if (userId === session.userId) {
    return index.me(yggdrasil, session);
  }

  const repoUser = await yggdrasil.repositories.users.get(userId);

  if (!canAccessUser(yggdrasil, session.user, repoUser.body)) {
...
index.set = (yggdrasil, session, data) => {
  if (!canAccessUser(yggdrasil, session.user, data)) {
...
index.create = (yggdrasil, session, data) => {
  if (!canAccessUser(yggdrasil, session.user, data)) {
...
index.findDupes = (yggdrasil, session, data) => {
...
index.update = (yggdrasil, session, data) => {
  const userId = data.objectId || data.userId || data;
 ...
 */
index.deleteAttachment = async (yggdrasil, session, data) => {
  const repoUser = await index.get(yggdrasil, session, data);
  ...
  return index.update(yggdrasil, session, userObject);
};
 ...
 */
index.delete = async (yggdrasil, session, data) => {
  const userId = data.objectId || data.userId || data;
  ...
  if (canAccessUser(yggdrasil, user, repoUser)) {
    return yggdrasil.repositories.users.delete(userId);
  }
  errorMessage = `${session.userId}, having policies ${JSON.stringify(session.user.data.policies)} is not allowed to delete user ${repouser.data._id} having policies ${JSON.stringify(repoUser.data.policies)}`;
  yggdrasil.logger.info(errorMessage);
  error = new Error(errorMessage);
  error.reason = 'FORBIDDEN';
  throw error;
 ...
 * @returns {*|PromiseLike<T | never>|Promise<T | never>}
 */
index.deleteOtherDupes = (yggdrasil, session, data) => {
  let promises = [];
  promises.push(index.findDupes(yggdrasil, session, {
  ...
  return Bluebird.all(promises)
    .then(results => {
      ...
      [...new Set([].concat(...results.map(s=>{return s.list;})))] // concat lists
        .filter(u => {
          return (u._id !== data.user._id && canAccessUser(yggdrasil, session.user, u));
        })
        .forEach(u => {
          // delete each accounts
          promises.push(index.delete(yggdrasil, session, u._id));
        });
