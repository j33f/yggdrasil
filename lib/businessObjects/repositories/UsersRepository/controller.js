...
const reduceList = (result, yggdrasil, session) => {
  ...
  result.list.forEach(user => {
    if (session.user.canAccessUser(user)) {
      response.push(user);
    }
  });
  return {
...
controller.get = async (yggdrasil, session, data) => {
  ...
  if (!session.user.canAccessUser(repoUser.body)) {
...
controller.set = (yggdrasil, session, data) => {
  if (!session.user.canAccessUser(data)) {
...
controller.delete = async (yggdrasil, session, data) => {
  ...
  errorMessage = `${session.userId}, having policies ${JSON.stringify(session.user.data.policies)} is not allowed to delete user ${repoUser.body._id} having policies ${JSON.stringify(repoUser.body.policies)}`;
  yggdrasil.logger.info(errorMessage);
  error = new Error(errorMessage);
  error.reason = 'FORBIDDEN';
  throw error;
