...
class MicrosoftStrategy {
  constructor(yggdrasil) {
    if (!yggdrasil.config.OAuth || !yggdrasil.config.OAuth.Microsoft) {
      yggdrasil.fire('log', 'warn', '🤷‍️The Microsoft OAuth strategy lacks of configuration information !');
            ...
            loggerCallback(loglevel, message) {
              switch (loglevel) {
                case msal.LogLevel.Error:
                  yggdrasil.fire('log', 'error', message);
                  break;
                case msal.LogLevel.Warning:
                  yggdrasil.fire('log', 'warn', message);
                  break;
              }
            },
  ...
  async authenticate(req, res, cb) {
    if (!cb) {
      ...
      try {
        ...
        this.yggdrasil.fire('log', 'error', 'MSAUTH - get redirect url', error);
        throw error;
      }
    }
    ...
    try {
      ...
      this.yggdrasil.fire('log', 'error', 'MSAUTH2', error);
      cb(null);
      throw error;
    }
    try {
      ...
      this.yggdrasil.fire('log', 'error', 'MSGraph', error);
      cb(null);
      throw error;
    }
  }
