...
class TwitterStrategy  {
  constructor(yggdrasil) {
    if (!yggdrasil.config.OAuth || !yggdrasil.config.OAuth.Twitter) {
      yggdrasil.logger.warn('🤷‍️The Twitter OAuth strategy lacks of configuration information !');
  ...
  async authenticate(req, res, cb) {
    if (!cb) {
      ...
      this.yggdrasil.logger.error('Twitter Auth - getRequestToken : callback not confirmed');
    ...
    try {
      ...
      this.yggdrasil.logger.error('Twitter OAuth', e);
      cb(null);
      throw e;
    }
  }
