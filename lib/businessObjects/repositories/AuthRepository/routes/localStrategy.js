...
router.get('/logout', async (req, res) => {
  if (res.yggdrasil.socketIoController) {
    res.yggdrasil.socketIoController.broadcast('logout/' + req.session.userId, null);
    res.yggdrasil.socketIoController.broadcast('logout', req.session.userId);
  }
...
router.post('/passwordResetRequest', async (req, res) => {
  ...
  try {
    ...
    res.yggdrasil.logger.error('Error during passwordResetRequest', e);
    res
      .status(404)
      .json({});
  }
...
router.post('/passwordReset', async (req, res) => {
  ...
  try {
    ...
    res.yggdrasil.logger.error('Error during passwordReset', e);
    res
      .status(401)
      .json({});
  }
