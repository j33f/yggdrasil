...
const checkStrategy = (req, res, strategy) => {
  if (!strategy) {
    ...
    req.yggdrasil.logger.warn(`The "${req.params.strategy}" OAuth strategy is not implemented (been accessed)`);
    return false;
  }
  if (!req.yggdrasil.config.OAuth || !req.yggdrasil.config.OAuth[strategy.name]) {
    commonResponses.OAuthNotImplemented(res);
    req.yggdrasil.logger.warn(`The "${req.params.strategy}" OAuth strategy is implementend but not configured (been accessed)`);
    return false;
  }
