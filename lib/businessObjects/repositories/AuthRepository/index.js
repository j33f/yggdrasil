...
class AuthRepository extends Repository {
  constructor(yggdrasil, name = 'Auth') {
    super(name, 'users', 'auth', yggdrasil, options);

    this.yggdrasil.OAuthStrategies = OAuthStrategies(yggdrasil);
    this.yggdrasil.fire('log', 'info', '🛂  OAuth strategies instantiated');
  }
  ...
  async deleteForUserId(id) {
    ...
    try {
      ...
      this.yggdrasil.fire('log', 'warn', e);
  ...
  async loginLocal(username, password) {
    ...
    const userCredentials = credentials.element.body;
    const credentialsId = credentials.element._id;
    ...
    if (!good && userCredentials.password !== password) {
      ...
      this.yggdrasil.fire('log', 'info', `❗Plain Password to encrypt for #${credentialsId}`);
      await this.setPassword(password, credentialsId);
    }
    ...
    if (userCredentials.impersonated) {
      this.yggdrasil.fire('log', 'warn', `User login successful for id ${userCredentials.userId} (impersonated)`);
    } else {
      this.yggdrasil.fire('log', 'info', `User login successful for id ${userCredentials.userId} (local)`);
    }
  ...
  async loginOAuth(data) {
    try {
      ...
      this.yggdrasil.fire('log', 'info', `User login successful for id ${userCredentials.userId} (OAuth)`);
