...
class FilesRepository extends Repository {
  constructor(yggdrasil, name = 'Files') {
    super(name, 'files', 'metadata', yggdrasil, {routes: routes});
   ...
   * @private
   */
  async _getFileFromHttp(options, retries = 0) {
    const response = await request(options);
    if (response.statusCode < 200 && response.statusCode >= 300) {
      if (retries < this.maxHttpGetRetries) {
        return Bluebird.delay(5000).then(this._getFileFromHttp(options, retries++));
      }
      throw new Error (`Cant get file form url ${options.uri}. Getting ${response.statusCode} (Retried ${this.maxHttpGetRetries} times`);
    }
    return response;
  }
  ...
  async _setFromUrl (id, url, metadata = {}) {
    ...
    await this._storeFile(id, metadata, requestResponse.body);
   ...
   */
  getFile(metadata) {
    return this.strategy.getFile(metadata);
  }
   ...
   * @returns {*}
   */
  async set(body, id, fileUrl = '', fileContent = '', isBase64 = false, encoding = null) {
    fileUrl = fileUrl || '';
   ...
   * @param isShared
   */
  async setShared (id, isShared = true) {
    let metadata = await super.get(id);
