...
stubbedSession = {
  ...
  withTransaction: sinon.stub().callsFake(fn => {
    return fn();
  }),
  ...
  describe('#Methods', () => {
    ...
    describe('#set', () => {
      ...
      it('should store the given document (not valid ID overridden)', async () => {
        ...
        driver._safeId = sinon.stub().returns(params._id);
    ...
    describe('#_getOneByQuery', () => {
      beforeEach(() => {
        fakeCollection.findOne = sinon.stub().resolves({_id: 'anId', body: {a: 'document'}});
