...
let driver, yggdrasil, stubbedConstructor, stubbedConnect, document;

stubbedConstructor = sinon.stub().callsFake((config=> console.log(config)));
stubbedConnect = sinon.stub().resolves();

class StubbedIORedis {
  constructor(config) {
...
describe('Redis Driver', () => {
  beforeEach(() => {
    ...
    driver = new Driver(yggdrasil, {foo: 'bar'}, null, StubbedIORedis);
    sinon.reset();
  });
  ...
  describe('#connect', () => {
    it('should call the connect method from the redis lib', async () => {
      ...
      should(stubbedConnect).have.been.called();
    });

    it('should driver.client initialized', async () => {
      await driver.connect();
      should(driver.isConnected).be.true();
    });

    it('should retry on bad connexion to redis', async () => {
      driver = new Driver(yggdrasil, {foo: 'bar'}, 3, StubbedIORedis);
      driver.client.connect = sinon.stub().onFirstCall().rejects().onSecondCall().resolves();
      await driver.connect();

      should(driver.client.connect).have.been.calledTwice();
    });

    it('should rejects on bad connexion to redis at maxRetries', async () => {
      driver = new Driver(yggdrasil, {foo: 'bar'}, 0, StubbedIORedis);
      driver.client.connect = sinon.stub().rejects(new Error('a message'));
      await driver.connect().should.be.rejectedWith(Error, {
        message: 'Cant connect to Redis',
        redisError: new Error('a message')
      });
    });
  });
  ...
  describe('#disconnect', () => {
    it('should call the disconnect method from the redis lib', () => {
      driver.client.disconnect = sinon.stub().resolves();
      driver.disconnect();
      should(driver.client.disconnect).have.been.called();
      should(yggdrasil.logger.info).have.been.calledWith('🔌  Redis is disconnected');
    });
  });
  ...
  describe('#get', () => {
    beforeEach(async () => {
      ...
      driver.client.get = sinon.stub().resolves(JSON.stringify(document.body));
    });
    ...
    it('should rejects if the document is not found', async () => {
      driver.client.get = sinon.stub().resolves(null);
  ...
  describe('#set', () => {
    beforeEach(async () => {
      ...
      driver.client.set = sinon.stub().resolves('OK');
    });
  ...
  describe('#delete', () => {
    beforeEach(async () => {
      ...
      driver.client.del = sinon.stub().resolves();
    });
  ...
  describe('#getList', () => {
    beforeEach(async () => {
      ...
      driver.client.get = sinon.stub().resolves(JSON.stringify(document.list));
    });
  ...
  describe('#flush', () => {
    beforeEach(async () => {
      driver.client.flushdb = sinon.stub().resolves();
    });
