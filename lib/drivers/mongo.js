...
class Mongo {
  constructor (yggdrasil, config, index, collection) {
    ...
    this.yggdrasil.config.mongo.retries = this.yggdrasil.config.mongo.retries || 30;
    this.yggdrasil.config.mongo.retryDelay = this.yggdrasil.config.mongo.retryDelay || 1000;
  }
  ...
  tryConnect() {
    const
      connectClient = Bluebird.promisify(MongoClient.connect),
      url = format(
        'mongodb://%s%s:%d/?ssl=%s',
    ...
    return connectClient(url, {
      ...
      auto_reconnect: this.options.autoReconnect || true,
      family: this.options.family || null,
      native_parser: this.options.useNativeParser || true,
      promiseLibrary: Bluebird,
      yggdrasilname: 'MCICRMV2',
      ssl: this.options.useSSL || 'false',
      sslValidate: this.options.sslValidate || false
    })
      .catch((e) => {
        if (me.connectionRetries < yggdrasil.config.mongo.retries) {
          yggdrasil.logger.info('☠  Can\'t connect to mongo, retrying...');
          me.connectionRetries++;
          return Bluebird.delay(yggdrasil.config.mongo.retryDelay)
            .then(() => me.tryConnect());
        }
        yggdrasil.logger.info(`☠  Cant connect to mongo, retried ${yggdrasil.config.mongo.retries} times with no success`);
  ...
  connect() {
    ...
    return this.tryConnect()
      .then(client => {
        yggdrasil.logger.info('⚡ Mongo Connected.');

        this.client = client;

        process.on('SIGINT', () => {
          client.close(false, () => {
            yggdrasil.logger.info('Mongo session closed');
          });
        });
        if (this.index) {
        ...
        this.startSession(yggdrasil.config.mongo.sessionConfig || undefined);
  ...
  selectDbCollection(index, collection) {
    ...
    this.yggdrasil.logger.info(`Mongo is not connected... Mongo is not connected... retry in ${this.yggdrasil.config.mongo.retryDelay / 1000}s`);
    this.connectionRetries++;
    if (this.connectionRetries === this.yggdrasil.config.mongo.retries) {
      return Bluebird.reject(new Error('Can\'t connect to Mongo...'));
    }
    return Bluebird.delay(this.yggdrasil.config.mongo.retryDelay)
   ...
   */
  set(params) {
    let id = params.id || params._id || this.yggdrasil.uuid();
  ...
  get(params) {
    ...
    const data = {
      ...
      query: {
        _id: this.safeId(params._id)
      }
    };
    return this.getOneByQuery(data);
  }
  ...
  getOneByQuery(params) {
    return this.selectDbCollection(params.index, params.collection)
      .then(collection => {
        return collection.findOne(params.query || {}, {session: this.session});
      })
      .then(document => {
        if (document === null) {
  ...
  deleteOneByQuery(params) {
    return this.selectDbCollection(params.index, params.collection)
      .then(collection => {
        return collection.deleteOne(params.query || {}, {session: this.session});
      });
  }
  ...
  list(params) {
    return this.selectDbCollection(params.index, params.collection)
      ...
      .then(cursor => {
        return Bluebird.resolve(cursor.toArray());
      });
  }
  ...
  bulk(params) {
    return this.selectDbCollection(params.index, params.collection)
      .then(collection => {
        return collection.bulkWrite(params.body, {session: this.session}); // transmute legacy Promise into Bluebird
      })
      .catch(e => {
        throw e;
      });
  ...
  createIndexes(params) {
    return this.selectDbCollection(params.index, params.collection)
      .then(collection => {
        return collection.createIndex(params.indexes, {session: this.session});
      });
  }
  ...
  getDistinct(params) {
    if (params.key === null) {
      return Bluebird.reject('You must provide a key from which to get distinct values');
    }
    return this.selectDbCollection(params.index, params.collection)
      .then(collection => {
        return collection.distinct(params.key, params.query, {session: this.session});
      });
  }
