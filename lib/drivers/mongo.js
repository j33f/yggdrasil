...
class Mongo {
  constructor(yggdrasil, config, index, collection, mongoConnect) {
    ...
    const protocol = ((this.options.protocol) ? this.options.protocol : 'mongodb')
    const auth = ((this.options.auth) ? this.options.auth.userName + ':' + this.options.auth.password + '@' : '');
    const host = this.options.host + ((this.options.port) ? ':' + this.options.port : '');

    this.mongoUrl = format(
      '%s://%s%s/?retryWrites=true&writeConcern=majority&ssl=%s',
      protocol,
      auth,
      host,
    ...
    this.mongoConnectOptions = {
      ...
      appname: this.yggdrasil.config.serviceName,
  ...
  async connect() {
    ...
    return this.yggdrasil.lib.utils.retry(
      () => {
        const ret = this.mongoConnect(this.mongoUrl, this.mongoConnectOptions);
        console.log(ret);
        return ret;
      },
      {
   ...
   */
  set(params) {
    let id = this._safeId(params._id || this.yggdrasil.uuid());
