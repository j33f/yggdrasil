...
async function startup(yggdrasil) {
      ...
      express: {
        ...
        router: {
          staticFiles: async () => {
            ...
            yggdrasil.use('/fs', express.static(path.join(yggdrasil.rootPath, 'fileStorage')));
            yggdrasil.logger.info('⚙  Will serve static files from', path.join(yggdrasil.rootPath, 'fileStorage'), 'as /fs');
          },
          API: async () => {
            /**
  ...
  async function configureYggdrasilForFunctionalTesting (_yggdrasil) {
    ...
    Object.keys(yggdrasil.repositories.plugins).forEach(key => {
      Object.keys(yggdrasil.repositories.plugins[key]).forEach(repo => {
        _yggdrasil.repositories.plugins[key][repo].setIndexCollection('testing_' + yggdrasil.repositories.plugins[key][repo].index);
        _yggdrasil.logger.info('!!! injecting fixtures into', _yggdrasil.repositories.plugins[key][repo].index, _yggdrasil.repositories.plugins[key][repo].collection);
      });
    });
